...
::Left4Bots.Events.OnGameEvent_round_start <- function(params) {
		...
		// 注意：当启用 scriptedmode 且未设置该导演选项时，主机会出现明显卡顿
		// 场景：女巫追逐幸存者且该幸存者进入安全屋。为该键设置任意值即可消除卡顿
		...
		// 开始接收用户命令
		::HooksHub.SetChatCommandHandler("l4b", ::Left4Bots.HandleCommand.bindenv(::Left4Bots));::HooksHub.SetConsoleCommandHandler("l4b", ::Left4Bots.HandleCommand.bindenv(::Left4Bots));::HooksHub.SetAllowTakeDamage("L4B", ::Left4Bots.AllowTakeDamage.bindenv(::Left4Bots));
	...
	::Left4Bots.Events.OnGameEvent_player_spawn <- function(params) {
		...
		Left4Timers.AddTimer(null, 0.01, @(params)::Left4Bots.OnPostPlayerSpawn.bindenv(::Left4Bots)(params.player, params.userid), {
			player = player,
			userid = params["userid"].tointeger()
		});
	}
	...
	::Left4Bots.Events.OnGameEvent_player_death <- function(params) {
		...
		if (victimIsPlayer)
			...
			victimName = victim.GetClassname(); // 变量名是 victimName，但若不是玩家则这里是实体的类名
		...
		if (attacker) {
			if (attackerIsPlayer)
				...
				attackerName = attacker.GetClassname(); // 变量名是 attackerName，但若不是玩家则这里是实体的类名
		}
	...
	::Left4Bots.Events.OnGameEvent_tank_killed <- function(params) {
		...
		if (tankId in ::Left4Bots.Tanks) {
			...
			if (Left4Bots.Tanks.len() == 0) // 所有 Tank 均已死亡
	...
	::Left4Bots.Events.OnGameEvent_player_disconnect <- function(params) {
		if ("userid" in params) {
			...
			if (player.GetZombieType() != 9) // 处理被踢/被删除的特感（各种原因）
			{
			...
			} else if (NetProps.GetPropInt(player, "m_iTeamNum") == TEAM_L4D1_SURVIVORS) // 同样处理被踢/被删除的一代幸存者
			{
	...
	::Left4Bots.Events.OnGameEvent_player_bot_replace <- function(params) {
		...
		if (Left4Bots.IsValidSurvivor(bot)) {
			local botUserID = params["bot"].tointeger();::Left4Bots.Survivors[botUserID] <- bot;::Left4Bots.Bots[botUserID] <- bot;::Left4Bots.SurvivorFlow[botUserID] <- {
				isBot = true,
				inCheckpoint = ::Left4Bots.IsSurvivorInCheckpoint(bot),
				flow = GetCurrentFlowDistanceForPlayer(bot)
			};
	...
	::Left4Bots.Events.OnGameEvent_bot_player_replace <- function(params) {
		...
		if (Left4Bots.IsValidSurvivor(player)) {
			local userid = params["player"].tointeger();::Left4Bots.Survivors[userid] <- player;::Left4Bots.SurvivorFlow[userid] <- {
				isBot = false,
				inCheckpoint = ::Left4Bots.IsSurvivorInCheckpoint(player),
				flow = GetCurrentFlowDistanceForPlayer(player)
			};
		}
	...
	::Left4Bots.Events.OnGameEvent_item_pickup <- function(params) {
		...
		if ((item == "pain_pills" || item == "adrenaline") && Left4Bots.IsHandledBot(player)) // Added IsHandledBot to fix https://github.com/smilz0/Left4Bots/issues/83
			Left4Timers.AddTimer(null, 1, @(params)::Left4Bots.CheckBotPickup.bindenv(::Left4Bots)(params.bot, params.item), {
				bot = player,
				item = "weapon_" + item
			});
	...
	::Left4Bots.Events.OnGameEvent_weapon_fire <- function(params) {
		...
		if (weapon == "pipe_bomb" || weapon == "vomitjar") {
			...
			if (params["userid"] in::Left4Bots.Bots) {
				...
				NetProps.SetPropInt(player, "m_fFlags", NetProps.GetPropInt(player, "m_fFlags") | (1 << 5)); // 设置 FL_FROZEN
		...
		// lxc：在 "weapon_fire" 事件中可让机器人朝向目标，与 BotThink_Main 中的 BotAim() 配合
	...
	::Left4Bots.Events.OnGameEvent_spit_burst <- function(params) {
		...
		if (Left4Bots.Settings.spit_block_nav)
			Left4Timers.AddTimer(null, 3.8, ::Left4Bots.SpitterSpitBlockNav.bindenv(::Left4Bots), {
				spit_ent = spit
			});
	}

	::Left4Bots.Events.OnGameEvent_zombie_death <- function(params) {
		switch (params["infected_id"]) // 普感=0，Smoker=1，Boomer=2，Hunter=3，Spitter=4，Jockey=5，Charger=6，Witch=7，Tank=8
		{
		...
		foreach(bot in ::Left4Bots.Bots) {
			if (bot.IsValid() && !Left4Bots.SurvivorCantMove(bot, bot.GetScriptScope().Waiting)) {
				...
				if (d <= 1200 /*&& Left4Utils.CanTraceTo(bot, charger, Left4Bots.Settings.tracemask_others)*/ ) {
					...
					Left4Timers.AddTimer(null, Left4Bots.Settings.dodge_charger_distdelay_factor * d, @(params)::Left4Bots.CheckShouldDodgeCharger.bindenv(::Left4Bots)(params.bot, params.charger, params.chargerOrig, params.chargerLeft, params.chargerForwardY), {
						bot = bot,
						charger = charger,
						chargerOrig = chargerOrig,
						chargerLeft = chargerLeft,
						chargerForwardY = chargerForwardY
					});
				}
			}
		}
	...
	::Left4Bots.Events.OnGameEvent_player_jump <- function(params) {
		...
		// 预期受害者应为特感当前盯视的幸存者，但若有其他幸存者挡在中间，则可能被误判为受害者（且不会尝试拦停特感）
		...
		if (d <= 150) {
			victim.GetScriptScope().BotSetAim(AI_AIM_TYPE.Shove, player, 0.233);::Left4Bots.PlayerPressButton(victim, BUTTON_SHOVE);
		} else {
			DoEntFire("!self", "RunScriptCode", @ "
				if (activator && !activator.IsDead() && !activator.IsDying() && !activator.IsStaggering()) {
					local nextshove = NetProps.GetPropFloat(self, ""
						m_flNextShoveTime "");
					if (Time() >= nextshove) {
		...
		::Left4Bots.Events.OnGameEvent_player_entered_checkpoint <- function(params) {
				...
				if (!::Left4Bots.AntiPipebombBugSetup && player && door && player.IsValid() && door.IsValid() && GetCurrentFlowPercentForPlayer(player) >= 85) {
					...
					EntityOutputs.AddOutput(door, "OnClose", "worldspawn", "RunScriptCode", "::Left4Bots.HandleAntiPipebombBug()", 0, -1);::Left4Bots.AntiPipebombBugSetup = true;
				}
			...
			::Left4Bots.Events.OnGameEvent_revive_begin <- function(params) {
				...
				if (((Left4Bots.Settings.throw_pipebomb && itemClass == "weapon_pipe_bomb") || (Left4Bots.Settings.throw_vomitjar && itemClass == "weapon_vomitjar")) &&
					...
					if (pos && (pos - player.GetOrigin()).Length() >= Left4Bots.Settings.throw_nade_mindistance)
						Left4Timers.AddTimer(null, 0.1, @(params)::Left4Bots.CancelReviveAndThrowNade.bindenv(::Left4Bots)(params.bot, params.subject, params.pos), {
							bot = player,
							subject = g_MapScript.GetPlayerFromUserID(params["subject"]),
							pos = pos
						});
				}
			}
			...
			::Left4Bots.Events.OnGameEvent_heal_begin <- function(params) {
				...
				{
					// 不允许幸存者机器人在以下情况下自我治疗：
					// - 当前生命值 >= Left4Bots.Settings.min_start_health（通常在起始安全屋会发生）且周围没有足够的医疗包
					// - 队伍中有真人玩家（否则他们可能不会离开安全屋）
					// - 当前不是“heal”指令
					if (player.GetHealth() >= Left4Bots.Settings.heal_interrupt_minhealth && Left4Bots.Bots.len() < Left4Bots.Survivors.len() && (NetProps.GetPropInt(player, "m_afButtonForced") & BUTTON_ATTACK) == 0 && !Left4Bots.HasSpareMedkitsAround(player))
						...
						// 强制持续治疗：避免因“感觉不安全”而中断，导致一直不治疗直至死亡

						Left4Bots.Logger.Debug(player.GetPlayerName() + " FORCE HEAL");

						::Left4Bots.PlayerPressButton(player, BUTTON_ATTACK, 0, null, 0, 0, true); // <- 若不锁视角，原版 AI 可能会打断治疗
					}
				}
			}
			...
			::Left4Bots.Events.OnGameEvent_finale_win <- function(params) {
				...
				foreach(id, bot in ::Left4Utils.GetAllSurvivors()) {
					if (bot && bot.IsValid() && IsPlayerABot(bot) && RandomInt(1, 100) <= Left4Bots.Settings.chat_gg_chance) {
						...
						if (NetProps.GetPropInt(bot, "m_lifeState") == 0 /* 存活？ */ && !bot.IsIncapacitated())
							linesToPickFrom = Left4Bots.ChatGGLines;

						if (linesToPickFrom.len() > 0)
							Left4Timers.AddTimer(null, RandomFloat(1.0, 7.0), @(params)::Left4Bots.SayLine.bindenv(::Left4Bots)(params.bot, params.line), {
								bot = bot,
								line = linesToPickFrom[RandomInt(0, linesToPickFrom.len() - 1)]
							});
					}
				}
			}
					...
					if (helloTriggers.find("," + text.tolower() + ",") != null) {
						...
						foreach(bot in Left4Bots.Bots) {
							if (RandomInt(1, 100) <= Left4Bots.Settings.chat_hello_chance)
								Left4Timers.AddTimer(null, RandomFloat(2.5, 6.5), @(params)::Left4Bots.SayLine.bindenv(::Left4Bots)(params.bot, params.line), {
									bot = bot,
									line = Left4Bots.ChatHelloReplies[RandomInt(0, Left4Bots.ChatHelloReplies.len() - 1)]
								});
						}
			...
			::Left4Bots.OnPostPlayerSpawn <- function(player, userid) {
				...
				if (IsValidSurvivor(player)) {
					...
					SurvivorFlow[userid] <- {
						isBot = false,
						inCheckpoint = IsSurvivorInCheckpoint(player),
						flow = GetCurrentFlowDistanceForPlayer(player)
					};
			...
			::Left4Bots.OnShovePressed <- function(params) {
				...
				if (!victimItem && slot == INV_SLOT_THROW) {
					...
					Left4Timers.AddTimer(null, 0.3, ::Left4Bots.ItemGiven.bindenv(::Left4Bots), {
						player1 = attacker,
						item = attackerItem,
						player2 = victim
					});
					...
					if (lvl >= Settings.userlevel_give_others) {
						...
						if (victimItemClass != attackerItemClass || victimItemSkin != attackerItemSkin) {
							...
							Left4Timers.AddTimer(null, 0.3, ::Left4Bots.ItemSwapped.bindenv(::Left4Bots), {
								item1 = victimItem,
								player1 = attacker,
								item2 = attackerItem,
								player2 = victim,
						...
						if ((surv.GetButtonMask() & BUTTON_SHOVE) != 0 || (NetProps.GetPropInt(surv, "m_afButtonPressed") & BUTTON_SHOVE) != 0) // <- With med items (pills and adrenaline) the shove button is disabled when looking at teammates and GetButtonMask never sees the button down but m_afButtonPressed still does
						{
							if (!(userid in BtnStatus_Shove) || !BtnStatus_Shove[userid]) {
								...
								if (Settings.give_humans_nades || Settings.give_humans_meds)
									Left4Timers.AddTimer(null, 0.0, ::Left4Bots.OnShovePressed.bindenv(::Left4Bots), {
										player = surv
									});
							}
							continue;
						...
						case "SurvivorBotHasRegroupedWithTeam":
							if (isHandledBot && Settings.deploy_upgrades) // Only Main bots
							{
								...
								if (itemClass) {
									...
									Left4Timers.AddTimer(null, 1, @(params)::Left4Bots.DoDeployUpgrade.bindenv(::Left4Bots)(params.player), {
										player = who
									});
								}
							}
						...
						case "SurvivorBotHasEscapedFlames":
							...
							if (!scope.CanReset) {
								...
								if (scope.DelayedReset)
									Left4Timers.AddTimer(null, 0.01, @(params) params.scope.BotReset(true), {
										scope = scope
									});
						...
						if (concept == "PlayerLook" || concept == "PlayerLookHere") {
							...
							if ("smartlooktype" in query && query.smartlooktype == "manual") {
								...
								if (IsHandledBot(subject)) {
									VocalizerBotSelection[who.GetPlayerUserId()] <- {
										bot = subject,
										time = Time()
									};
