...
enum AI_MOVE_TYPE {
	...
	::Left4Bots.BotOrderPrepare <- function(bot, orderType, from = null, destEnt = null, destPos = null, destLookAtPos = null, holdTime = 0.0, canPause = true, param1 = null) {
		...
		local order = {
			OrderType = orderType,
			Priority = OrderPriorities[orderType],
			From = from,
			DestEnt = destEnt,
			DestPos = destPos,
			DestLookAtPos = destLookAtPos,
			HoldTime = holdTime,
			CanPause = canPause,
			Param1 = param1
		};
	...
	::Left4Bots.AIFuncs.BotThink_Door <- function() {
		...
		if ((Origin - doorPos).Length() <= L4B.Settings.move_end_radius_door || L4B.CanUseTo(self, DoorEnt)) {
			...
			Left4Timers.AddTimer(null, L4B.Settings.door_failsafe_delay, @(params)::Left4Bots.DoorFailsafe.bindenv(::Left4Bots)(params.bot, params.door, params.action), {
				bot = self,
				door = DoorEnt,
				action = DoorAct
			});
	...
	::Left4Bots.AIFuncs.BotFinalizeCurrentOrder <- function() {
		...
		switch (CurrentOrder.OrderType) {
			...
			case "deploy": {
				...
				Left4Timers.AddTimer(null, 1, @(params) params.bot.SwitchToItem(params.cls), {
					bot = self,
					cls = Left4Utils.StringReplace(CurrentOrder.DestEnt.GetClassname(), "_spawn", "")
				});

				Left4Timers.AddTimer(null, 2, @(params)::Left4Bots.DoDeployUpgrade.bindenv(::Left4Bots)(params.bot), {
					bot = self
				});
					...
					if (ActiveWeapon && ActiveWeapon.GetClassname() == "weapon_first_aid_kit") {
						...
						if (CurTime > NetProps.GetPropFloat(ActiveWeapon, "m_flNextPrimaryAttack") + 0.1) // <- Add a little delay or the animation will be bugged
						{
							...
							if (CurrentOrder.DestEnt.GetPlayerUserId() == self.GetPlayerUserId())
								...
								Left4Timers.AddTimer(null, 0.8, @(params)::Left4Bots.CheckHealingTarget.bindenv(::Left4Bots)(params.bot, params.order), {
									bot = self,
									order = CurrentOrder
								});
							}
		...
		if (reason == true)
			...
			if (reason.GetClassname() == "witch") {
				...
				switch (reason.GetZombieType()) {
					case Z_SMOKER: {
						Left4Timers.AddTimer(null, 0.01, @(params)::Left4Utils.BotCmdAttack(params.bot, params.target), {
							bot = self,
							target = reason
						});
