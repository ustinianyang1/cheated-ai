...
::Left4Bots.Initialize <- function() {
		...
		Left4Utils.LoadSettingsFromFileNew("cfg/settings.txt", "Left4Bots.Settings.", Logger);

		Logger.LogLevel(Settings.loglevel);

		Left4Utils.SaveSettingsToFileNew("cfg/settings.txt", Settings, Logger);
		...
		if (Left4Utils.FileExists("cfg/ontank_settings.txt")) {
			Logger.Info("Loading OnTank settings...");
			Left4Utils.LoadSettingsFromFileNew("cfg/ontank_settings.txt", "Left4Bots.OnTankSettings.", Logger, true);
		}
		Left4Utils.PrintSettingsNew(OnTankSettings, Logger, "[OnTank Settings] ");

		if (Left4Utils.FileExists("cfg/ontank_convars.txt")) {
			Logger.Info("Loading OnTank convars...");
			local c = LoadOnTankCvarsFromFile("cfg/ontank_convars.txt");
	...
	::Left4Bots.LoadSettingsOverride <- function() {
		...
		local fileName = "cfg/settings_" + MapName + "_" + Difficulty + "_" + ModeName + ".txt"
		...
		fileName = "cfg/settings_" + Difficulty + "_" + ModeName + ".txt"
		if (Left4Utils.LoadSettingsFromFileNew(fileName, "Left4Bots.Settings.", Logger)) {
			Logger.Info("Loaded settings overrides from: " + fileName);
			return;
		}
		...
		fileName = "cfg/settings_" + MapName + "_" + ModeName + ".txt"
		if (Left4Utils.LoadSettingsFromFileNew(fileName, "Left4Bots.Settings.", Logger)) {
			Logger.Info("Loaded settings overrides from: " + fileName);
			return;
		}
		...
		fileName = "cfg/settings_" + ModeName + ".txt"
		if (Left4Utils.LoadSettingsFromFileNew(fileName, "Left4Bots.Settings.", Logger)) {
			Logger.Info("Loaded settings overrides from: " + fileName);
			return;
		}
		...
		fileName = "cfg/settings_" + MapName + "_" + Difficulty + ".txt"
		if (Left4Utils.LoadSettingsFromFileNew(fileName, "Left4Bots.Settings.", Logger)) {
			Logger.Info("Loaded settings overrides from: " + fileName);
			return;
		}
		...
		fileName = "cfg/settings_" + Difficulty + ".txt"
		if (Left4Utils.LoadSettingsFromFileNew(fileName, "Left4Bots.Settings.", Logger)) {
			Logger.Info("Loaded settings overrides from: " + fileName);
			return;
		}
		...
		// 7. settings_[map].txt
		fileName = "cfg/settings_" + MapName + ".txt"
		if (Left4Utils.LoadSettingsFromFileNew(fileName, "Left4Bots.Settings.", Logger)) {
			Logger.Info("Loaded settings overrides from: " + fileName);
			return;
		}
	...
	::Left4Bots.AddonStop <- function() {
		...
		::HooksHub.RemoveChatCommandHandler("l4b");::HooksHub.RemoveConsoleCommandHandler("l4b");::HooksHub.RemoveAllowTakeDamage("L4B");
	...
	::Left4Bots.FindBotNearestEnemy <- function(bot, orig, radius, minDot = 0.96) {
		...
		foreach(ret_data in ret_array) {
			...
			if (ret && Left4Utils.CanTraceTo(bot, ret, tracemask_others)) {
				return {
					ent = ret,
					head = ret_data[0] <= Settings.manual_attack_special_head_radius
				};
			}
		}
		...
		foreach(ret_data in ret_array) {
			...
			if (ret && (Settings.manual_attack_wandering || IsInfectedAngry(ret)) && !IsRiotPolice(ret, orig) && Left4Utils.CanTraceTo(bot, ret, tracemask_others)) {
				return {
					ent = ret,
					head = ret_data[0] <= Settings.manual_attack_common_head_radius
				};
			}
		}
...
if (self.GetClassname().find(""
		_spawn ""))
	isWorthPickingUp = NetProps.GetPropInt(self, ""
		m_itemCount "") > 0;
if (activator && isWorthPickingUp) {
	...
	Left4Bots.Logger.Debug(""
		PickupFailsafe - "" + activator.GetPlayerName() + "" - > "" + self + ""("" + weaponid + "")
		"");
...
::Left4Bots.CheckShouldDodgeCharger <- function(bot, charger, chargerOrig, chargerLeft, chargerForwardY) {
	...
	::Left4Bots.CheckHealingTarget <- function(bot, order) {
		...
		if (!IsPlayerABot(bot) || !target || !order.DestEnt || !order.DestEnt.IsValid() || target.GetPlayerUserId() != order.DestEnt.GetPlayerUserId()) {
			...
			if (IsPlayerABot(bot))
				Left4Timers.AddTimer(null, 0.5, @(params)::Left4Bots.BotOrderRetry.bindenv(::Left4Bots)(params.bot, params.order), {
					bot = bot,
					order = order
				});
		}
	...
	*/
	...
	::Left4Bots.CanTraceToPickup <- function(bot, item) {
		...
		local traceTable = {
			...
			ignore = bot,
			mask = Settings.tracemask_pickups
		};
	...
	::Left4Bots.GetAvailableScavengeItems <- function() {
		...
		while (ent = Entities.FindByModel(ent, model)) {
			if (ent.IsValid() && (Settings.scavenge_pour || (ent.GetOrigin() - ScavengeUseTarget.GetOrigin()).Length() >= Settings.scavenge_drop_radius) && IsValidPickup(ent) && !BotsHaveOrderDestEnt(ent))
				t[++i] <- {
					ent = ent,
					flow = GetFlowDistanceForPosition(ent.GetOrigin())
				};
		}
		return t;
	}
	...
	::Left4Bots.SpitterSpitBlockNav <- function(params) {
		...
		local kvs = {
			...
			origin = spit.GetOrigin(),
			extent = Vector(Settings.dodge_spit_radius, Settings.dodge_spit_radius, Settings.dodge_spit_radius),
			teamToBlock = "2",
			affectsFlow = "0"
		};
	...
	::Left4Bots.CancelReviveAndThrowNade <- function(bot, subject, pos) {
		...
		//BotThrow(bot, pos);
		Left4Timers.AddTimer(null, 0.01, @(params)::Left4Bots.BotThrow.bindenv(::Left4Bots)(params.bot, params.pos), {
			bot = bot,
			pos = pos
		});
	}
	...
	::Left4Bots.ValidWeaponForSmoker <- function(weapon) {
		...
		local allowed = [".*pistol.*", ".*smg.*"
			".*rifle.*", ".*shotgun.*", ".*sniper.*" /*, ".*grenade_launcher.*"*/
		];
	...
	::Left4Bots.ModeName = Director.GetGameMode();::Left4Bots.BaseModeName = Director.GetGameModeBase();::Left4Bots.MapName = Director.GetMapName();::Left4Bots.Difficulty = Convars.GetStr("z_difficulty").tolower();::Left4Bots.SurvivorSet = Director.GetSurvivorSet();
