...
Base Task.Assigns a single 'order'
to the 'target'
bot(s)
...
::Left4Bots.Automation.Events.OnGameEvent_player_entered_checkpoint <- function(params) {
	...
	::Left4Bots.Automation.step = 999999;::Left4Bots.Automation.CurrentTasks.clear();
	if (!::Left4Bots.Settings.automation_stay_in_end_saferoom)
		return;

	local mapAreas = {};::Left4Utils.FindMapAreas(mapAreas);
	if (mapAreas["checkpointB_in"]) {
		::Left4Bots.Settings.move_end_radius_wait = 30;::Left4Bots.Automation.DoWait("bots", mapAreas["checkpointB_in"].GetCenter());
	}
}